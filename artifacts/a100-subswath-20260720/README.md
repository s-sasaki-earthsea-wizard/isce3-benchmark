...
The sinc chip-buffer device transient model (n_elem_out × 81 × 8 B)
is CONFIRMED by the probe — and the transient is so short-lived
(~tens of ms) that 20–50 ms samplers miss it. lpb=1000 on this
geogrid materializes a 16.9 GiB peak → exceeds an RTX 5080's total
16.3 GiB VRAM; the consumer-OOM claim is now measured, not estimated.
